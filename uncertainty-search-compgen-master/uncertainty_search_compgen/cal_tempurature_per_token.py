...
train_only_trainer, train_only_model, train_only_harness = finetune_tf_lm(
    ...
    train_full,
    val_pairs,
    load_from="logs/train_only/stage2/final_model_with_new_tempurature_stage2.ckpt",  
    do_fit=False 
)
train_only_harness.train_mode = "ats"
print(f"Train mode: {train_only_harness.train_mode}")
...
with torch.no_grad():   
    for i, batch in enumerate(test_data):
        ...
        all_token_temperature.append(temperatures)
        all_token_losses.append(token_losses.cpu())
        all_labels_ref.append(labels.cpu()) # save labels

all_token_temperature_tensor = torch.cat(all_token_temperature, dim=0)
...
for sample_idx in range(len(all_token_losses_tensor)):
    ...
    sample_temperature = all_token_temperature_tensor[sample_idx]

    for token_idx, (loss, label_id, temperature) in enumerate(zip(sample_losses, sample_labels,sample_temperature)):
        if label_id != -100:  
            token = tokenizer.decode(label_id.item())
            results.append({
                'token_idx': token_idx,
...
# 保存为JSON
import json
with open('token_losses_readable_sorted_with_temperature_with_original_logits.json', 'w') as f:
    json.dump(results, f, indent=2)

import pandas as pd; df = pd.read_json(path_to_json); print(df[‘loss’].corr(df[‘temperature’]))